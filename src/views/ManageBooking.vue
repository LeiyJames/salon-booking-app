      ...
      <div v-if="filteredAppointments.length" class="space-y-3">
        <div v-for="apt in filteredAppointments" :key="apt.id" class="card p-4">
          ...
          <div v-if="apt.status !== 'cancelled'" class="flex gap-2">
            <button class="btn-danger text-xs px-3 py-2" @click="cancelApt(apt.id)">
              <XCircle :size="14" /> Cancel
            </button>
            <button class="btn-secondary text-xs px-3 py-2" @click="openReschedule(apt)">
      ...
      <teleport to="body">
        ...
        <div v-if="rescheduleApt" class="modal-content p-6">
          <div class="flex items-center justify-between mb-4">
            ...
            <button @click="rescheduleApt = null" class="p-2 rounded-lg hover:bg-dark-700 text-dark-400">
              <X :size="18" />
            </button>
          </div>
            ...
            <div v-if="rescheduleSlots.length > 0">
              ...
              <div class="grid grid-cols-3 gap-2 max-h-48 overflow-y-auto">
                <div
                  ...
                  :key="slot.time"
                  class="time-slot"
                  :class="{ 'time-slot-selected': newTime === slot.time }"
                  @click="newTime = slot.time; newEndTime = slot.endTime"
                >
                  {{ formatTime(slot.time) }}
                </div>
              </div>
            </div>
            <div v-else-if="newDate" class="text-center py-4 text-dark-400 text-sm">
          ...
          <button
            ...
            :class="{ 'opacity-50': !newDate || !newTime }"
            @click="doReschedule"
          >
            Confirm Reschedule
          </button>
  ...
  return store.appointments
    .filter(a => {
      ...
      return (
        ...
        (a.services && a.services.some(s => s.name.toLowerCase().includes(q)))
      )
    })
...
function doReschedule () {
  store.rescheduleAppointment(rescheduleApt.value.id, newDate.value, newTime.value, newEndTime.value)
  rescheduleApt.value = null
}
</script>
