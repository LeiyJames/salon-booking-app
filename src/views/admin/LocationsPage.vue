      ...
      <div v-for="loc in store.locations" :key="loc.id" class="card p-5">
        <div class="flex items-start gap-4">
          ...
          <div class="flex-1 min-w-0">
            ...
            <button
              v-if="store.currentLocationId !== loc.id"
              class="btn-secondary text-xs px-3 py-2 mt-3"
              @click="store.currentLocationId = loc.id"
            >
...
function locationStaffCount (locId) {
  return store.stylists.filter(s => s.locationIds.includes(locId)).length
}

function locationServiceCount (locId) {
  return store.services.filter(s => s.locationIds.includes(locId)).length
}
</script>
