  ...
  <div class="p-4 md:p-6 max-w-6xl mx-auto">
    ...
    <div class="flex gap-2 overflow-x-auto pb-3 mb-4">
      <button
        ...
        :class="activeFilter === f.value ? 'gradient-gold text-dark-900' : 'bg-dark-700 text-dark-300 hover:bg-dark-600'"
        ...
        <div class="flex items-start gap-4">
            ...
            <div class="flex items-center gap-2 mb-1">
              ...
              <span class="badge text-[10px]"
                :class="{
                  ...
                  'badge-accent': apt.paymentStatus === 'refunded',
          ...
          <div class="flex flex-col items-end gap-2 flex-shrink-0">
            ...
            <div class="flex gap-1" v-if="apt.status !== 'cancelled'">
              <button
                v-if="apt.paymentStatus === 'unpaid'"
                class="text-xs px-2 py-1 rounded-lg bg-green-500/10 text-green-400 hover:bg-green-500/20 transition"
                @click="store.markPayment(apt.id, 'paid')"
              >
                Mark Paid
              </button>
              <button
                class="text-xs px-2 py-1 rounded-lg bg-red-500/10 text-red-400 hover:bg-red-500/20 transition"
    ...
    <div v-else class="card p-8 text-center">
      <p class="text-dark-400">No appointments match the current filter.</p>
    </div>
  </div>
</template>
