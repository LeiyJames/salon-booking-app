<template>
  <div class="p-4 md:p-6 max-w-6xl mx-auto">
    <div class="mb-6 animate-fade-in-up">
      <h1 class="font-display text-2xl md:text-3xl font-bold text-dark-100 mb-1">Good {{ greeting }}, Admin</h1>
    ...
    <div class="animate-fade-in-up delay-300">
      <div class="flex items-center justify-between mb-4">
        ...
        <router-link to="/admin/appointments" class="text-gold-400 text-sm font-medium hover:text-gold-300 flex items-center gap-1">View All <ArrowRight :size="14" /></router-link>
      </div>
          ...
          <div class="flex items-center gap-4">
            ...
            <div class="flex-1 min-w-0">
              <div class="flex items-center gap-2 mb-0.5">
                ...
                <span class="badge text-[10px]" :class="{ 'badge-success': apt.paymentStatus === 'paid', 'badge-danger': apt.paymentStatus === 'unpaid' }">{{ apt.paymentStatus }}</span>
              </div>
    ...
    <div class="mt-6 grid grid-cols-2 md:grid-cols-4 gap-3 animate-fade-in-up delay-400">
      <router-link to="/book" class="card p-4 flex items-center gap-3 group card-interactive">
        <div class="w-10 h-10 rounded-xl bg-gold-500/10 flex items-center justify-center"><Plus :size="18" class="text-gold-400" /></div>
      ...
      <router-link to="/admin/clients" class="card p-4 flex items-center gap-3 group card-interactive">
        <div class="w-10 h-10 rounded-xl bg-accent-500/10 flex items-center justify-center"><UserPlus :size="18" class="text-accent-400" /></div>
      ...
      <router-link to="/admin/calendar" class="card p-4 flex items-center gap-3 group card-interactive">
        <div class="w-10 h-10 rounded-xl bg-dark-600 flex items-center justify-center"><CalendarDays :size="18" class="text-dark-300" /></div>
      ...
      <router-link to="/admin/services" class="card p-4 flex items-center gap-3 group card-interactive">
        <div class="w-10 h-10 rounded-xl bg-dark-600 flex items-center justify-center"><Sparkles :size="18" class="text-dark-300" /></div>
...
import { CalendarDays, ArrowRight, Plus, UserPlus, Sparkles, Users, DollarSign, TrendingUp } from 'lucide-vue-next'
