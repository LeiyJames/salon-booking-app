  ...
  <div class="p-4 md:p-6 max-w-6xl mx-auto">
    ...
    <div class="card p-4 mb-5">
      ...
      <div class="calendar-grid">
        <div v-for="(day, idx) in calendarDays" :key="idx" class="calendar-cell relative"
          :class="{ 'calendar-cell-selected': day.dateStr === selectedDate, 'calendar-cell-today': day.isToday && day.dateStr !== selectedDate, 'text-dark-100': day.day }"
    ...
    <div>
      ...
      <div v-if="dayAppointments.length === 0" class="card p-6 text-center"><p class="text-dark-400 text-sm">No appointments on this day.</p></div>
...
import { store, formatTime, formatDateLong } from '../../store'
...
const calendarDays = computed(() => {
  ...
  const cells = []
  for (let i = 0; i < firstDay.getDay(); i++) cells.push({ day: null, dateStr: '', isToday: false })
...
const dayAppointments = computed(() => store.appointments.filter(a => a.date === selectedDate.value && a.locationId === store.currentLocationId && a.status !== 'cancelled').sort((a, b) => a.time.localeCompare(b.time)))
function dayHasAppointments (dateStr) { return dateStr && store.appointments.some(a => a.date === dateStr && a.locationId === store.currentLocationId && a.status !== 'cancelled') }
